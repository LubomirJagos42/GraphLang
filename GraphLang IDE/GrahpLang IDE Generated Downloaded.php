...
<!--    <script src="--><?php //echo $htmlIncludeDirPrefix; ?><!--/GraphLangUtils/TranslateToCppCode_1.js"></script>--><!-- obsolete, replaced by 2nd translator which is object -->
    ...
    foreach ($orderedNodesList as $node){
        if ($nodesListWithCateories[$node['internal_id']]['nodeIsHidden']){
    ?>
        <script type="text/javascript" src="<?= $htmlIncludeDirPrefix ?>/LibraryBlocks/_hidden/<?= $node['node_display_name'].'.js' ?>"></script>
        <?php
        }else{
        ?>
        <script type="text/javascript" src="<?= $htmlIncludeDirPrefix ?>/LibraryBlocks/<?= $nodesListWithCateories[$node['internal_id']]['categoryName'].'/' ?><?= $node['node_display_name'].'.js' ?>"></script>
    <?php
        }
    }
    ?>
