...
// http://www.draw2d.org
//
GraphLang.Shapes.Basic.SelectOption = GraphLang.UserDefinedNode.extend({
    ...
    additionalContextMenuItems: {
        ...
        "editItems": {name: "Edit Items"},
    },

    additionalContextMenuItemsHandlerPostHook: function(emitter, key, options){
        switch(key){
            case "addItem":
                alert(`add item into select option: ${emitter.getId()}`);
                break;
            case "editItems":
                alert(`edit items into select option: ${emitter.getId()}`);
                break;
            default:
                break;
        }
    ...
    init:function(attr, setter, getter)
    {
        ...
        /***************************************************************************************************
         *  ADD ITEMS INTO SELECT OPTION
         ***************************************************************************************************/
        this.userData = {};
        this.getUserData().optionItemsArray = [{name: "some name", value: "some value"}];   //this is just example in which form to push values for option items into array

        if (this.getUserData() !== undefined && this.getUserData().optionItemsArray !== undefined){
            for (let optionItem of this.getUserData().optionItemsArray){
                this.additionalContextMenuItems[optionItem.name] = {name: optionItem.value};
            }
        }
