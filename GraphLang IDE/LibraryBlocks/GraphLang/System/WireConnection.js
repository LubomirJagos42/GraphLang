...
GraphLang.Shapes.Basic.WireConnection = GraphLang.Shapes.Basic.Tunnel.extend({
    ...
    init : function(attr)
    {
         ...
         */
        port = this.getInputPort(0);
        port.setId(this.getInputPort(0).getId().slice(0, -4));		//strip "-in0" from id
        ...
        port = this.getOutputPort(0)
        port.setId(this.getOutputPort(0).getId().slice(0, -5));	//strip "-out0" from id
    ...
    translateToCppCode: function(){
        ...
        outputPort.getConnections().each(function(outputPortWireIndex, outputPortWireObj){
            cCode += outputPortWireObj.getVariableName() + " = " + inputWire.getVariableName() + ";\n";
        });
