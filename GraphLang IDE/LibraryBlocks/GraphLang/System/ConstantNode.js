...
GraphLang.Shapes.Basic.ConstantNode = draw2d.shape.basic.Label.extend({
  ...
  init:function(attr, setter, getter){
    ...
    /*****************************************************************************
     *  ESCAPE TEXT before update it into node
     *****************************************************************************/
    let modifiedTextEditor = new draw2d.ui.LabelInplaceEditor({
        // called after the value has been set to the LabelFigure
        onCommit: $.proxy(function(value){
            /*
             *  Here could be handled text input validation before set to label
             */
            if (this.getDatatype().toLowerCase().search("string") > -1) {
                ...
                console.log(`--> ${this.NAME} (${this.getId()}), text changed to: ${value}`);
            }
            this.setText(value);
        },this),
        onCancel: function(){} // called if the user abort the operation
    });
    this.installEditor(modifiedTextEditor);

    //this is used when user double click on constant an start editing
    // this.on("change:text",function(emitter,event){
    //     emitter.text = JSON.parse(emitter.getText());   //this will not fire another event!
    // });
        ...
        $.contextMenu({
            ...
            callback: $.proxy(function(key, options)
            {
               ...
               case "std::complex<float>":
                   emitter.changeDatatype(key);
                   emitter.setText("1.0 + 1.0i");      //<-- complex number value
                   break;
               default:
