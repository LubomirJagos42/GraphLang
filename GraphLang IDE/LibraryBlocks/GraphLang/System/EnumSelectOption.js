...
GraphLang.Shapes.Basic.EnumSelectOption = draw2d.shape.basic.Label.extend({
    ...
    {
        this._super(memento);
        console.log(`--> loading enumselectoption`);
        console.log(memento);

        if (memento.userData.sourceFigureId){
            console.log(`----> loading EnumSelectOption using sourceFigureId to update editor`);
            console.log(memento.userData.sourceFigureId);
    ...
    /*
     *  TODO: Loading process not working, connectin to parent node is not loaded. Then select options are not working.
     */
    ...
    translateToCppCodeDeclaration: function(){
        var cCode = "";
        cCode += this.getDatatype() + " " + this.getVariableName() + " = " + this.getText() + ";\n";
        return cCode;
    },
